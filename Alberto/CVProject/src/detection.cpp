...
std::vector<cv::Rect> cascade(cv::Mat img)
{
	...
	cv::Mat output = img;
	my_cascade.detectMultiScale(img, hands, 1.3, 2, 0, cv::Size(100, 100), cv::Size(5500, 5500));//working pretty good
	//my_cascade.detectMultiScale(img, hands, 1.2, 4, 0, cv::Size(100, 100), cv::Size(300, 300)); //tested
	//my_cascade.detectMultiScale(img, hands, 1.2, 4, 0, cv::Size(50, 50), cv::Size(1500, 1500));//tested
	//my_cascade.detectMultiScale(img, hands, 1.3, 3, 0, cv::Size(100, 100), cv::Size(1500, 1500));
...
std::vector<float> templateMatching(cv::Mat img, std::vector<cv::Rect> hands, cv::Mat hand_img)
{
	...
	if (maxMaxVal < 0.045) // 0.5 works well with test image 01
	{
		std::vector<float> output;
		output.push_back(0.0);
		output.push_back(0.0);
